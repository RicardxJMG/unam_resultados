...
def server(input: Inputs, output: Outputs, session: Session):
    ...
    def careers_demand(): 
        ...
        fig.update_layout(legend = dict(title = 'Áreas'), yaxis = dict(range = [0,30000]))
        ...
        if "id_area" not in df_facultades.columns:
            fig.add_trace(
                go.Bar(
                    ...
                    showlegend=False,
                    width = 0.55, 
                    text =df_facultades.n_aspirantes,
                    textposition='outside', 
    ...
    def scatter_oda():
        ...
        fig.add_trace(
            trace = go.Histogram(
                x = df[x_axis], nbinsx= 30, marker_color = '#88c0d0'),
            row = 1, col=1
        )
        
        fig.add_trace(
            trace = go.Histogram( 
                y = df[y_axis], nbinsy=30, marker_color = '#88c0d0'),
            row = 2, col=2
        )
        ...
        fig.update_layout(margin=dict(t=50, r=50, b=50, l=50),
                  annotations=[
                      dict(text=SCATTER_SELECTOR[x_axis],
                           x=0.5, xref='paper', xanchor='center',
                           y=-0.05, yref='paper', yanchor='bottom',
                           showarrow=False, font=dict(size=14)),
                      dict(text=SCATTER_SELECTOR[y_axis],
        ...
        # probable mejora es añadir una linea de tendencia.
        return fig
